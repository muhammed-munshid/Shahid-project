...
function EditDiary({ id, onEditComplete, cancelPath }) {
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                const response = await axios.get(`${mainUrl}get-diary-by-id/${id}`);
                const { note1, note2, note3, note4, note5, note6, note7, note8, note9, note10, note11, note12, note13, note14, note15, note16,
                    note17, note18, note19, note20, note21, note22, note23, note24, note25, note26, note27, note28, note29, note30,
                    note31, note32, note33, note34, note35, note36, note37 } = response.data.data;
                setNote1(note1);
                setNote2(note2);
                setNote3(note3);
                setNote4(note4);
                ...
                console.log(error);
                toast.error('Failed to fetch diary data');
            }
        };
    ...
    const handleSubmit = (e) => {
        e.preventDefault();
        axios.put(`${mainUrl}edit-diary/${id}`, diaryData)
            .then((response) => {
                if (response.data.success) {
                    toast.success(response.data.message);
                    onEditComplete(response.data.data);
                } else {
            ...
            <Layout>
                <div className="flex justify-center items-center h-full px-4">
                    <div className="w-full max-w-3xl bg-white shadow-lg rounded-lg p-8">
                        <h2 className="text-2xl font-bold mb-6 text-center">Edit Diary</h2>
